...
% p_i   =  R_io * (p_o - t_oi)
% namely, 
% p_i   =  Rx(omega) * Ry(phi) * Rz(kappa) * (p_o - [t_x, t_y, t_z]'),
...
figure(2);
subplot(1,scan_count+1,1);
for i=1:scan_count
   plot_scanner(x_p(i*6-1:i*6+4),i,2);
end
scatter3(ops(:,1),ops(:,2),ops(:,3),plot_points_size,'m','filled');
xlabel('X(m)');
ylabel('Y(m)');
zlabel('Z(m)');
...
figure(2)
for i=1:scan_count
   subplot(1,scan_count+1,i+1);
   plot_scanner(zeros(6,1),i,2);
   ...
   scatter3(inliers(:,1),inliers(:,2),inliers(:,3),plot_points_size,'g','filled');
   scatter3(pausibility_outliers(:,1),pausibility_outliers(:,2),pausibility_outliers(:,3),plot_points_size,'r','filled');
   scatter3(dannish_outliers(:,1),dannish_outliers(:,2),dannish_outliers(:,3),plot_points_size,'b','filled');
   xlabel('X(m)');
   ylabel('Y(m)');
   zlabel('Z(m)');
